...
var _ = Describe("Manila webhook", func() {
	It("rejects update to deprecated rabbitMqClusterName field", func() {
		...
		Eventually(func(g Gomega) {
			g.Expect(k8sClient.Get(ctx, manilaName, unstructuredObj)).Should(Succeed())
			...
			g.Expect(statusError.ErrStatus.Details.Kind).To(Equal("Manila"))
	...
	It("rejects update to deprecated notificationsBusInstance field", func() {
		...
		manilaName := types.NamespacedName{
			Namespace: namespace,
			Name:      "manila-webhook-notif-test",
		}
