...
import (
	...
	manilascheduler "github.com/openstack-k8s-operators/manila-operator/internal/manilascheduler"
...
func (r *ManilaSchedulerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	if err != nil {
		...
		Log.Error(err, fmt.Sprintf("could not fetch ManilaScheduler instance %s", instance.Name))
...
func (r *ManilaSchedulerReconciler) reconcileNormal(ctx context.Context, instance *manilav1beta1.ManilaScheduler, helper *helper.Helper) (ctrl.Result, error) {
	...
	ssDef, err := manilascheduler.StatefulSet(instance, inputHash, serviceLabels, serviceAnnotations, topology, memcached)
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
