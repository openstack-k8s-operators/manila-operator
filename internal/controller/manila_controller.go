...
func (r *ManilaReconciler) generateServiceConfig(
	...
	if instance.Spec.ManilaAPI.Auth.ApplicationCredentialSecret != "" {
		acSecretObj, _, err := secret.GetSecret(ctx, h, instance.Spec.ManilaAPI.Auth.ApplicationCredentialSecret, instance.Namespace)
		if err != nil {
			if !k8s_errors.IsNotFound(err) {
				h.GetLogger().Error(err, "Failed to get ApplicationCredential secret", "secret", instance.Spec.ManilaAPI.Auth.ApplicationCredentialSecret)
			}
		} else {
			...
			if okID && len(acID) > 0 && okSecret && len(acSecretData) > 0 {
				...
				h.GetLogger().Info("Using ApplicationCredentials auth", "secret", instance.Spec.ManilaAPI.Auth.ApplicationCredentialSecret)
			}
		}
	}
