...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/validate-manila-openstack-org-v1beta1-manila
  failurePolicy: Fail
...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/mutate-manila-openstack-org-v1beta1-manila
  failurePolicy: Fail
  matchPolicy: Equivalent
  name: mmanila.kb.io
  objectSelector: {}
...
if [ -n "${CSV_NAME}" ]; then
    ...
    if CR_NAME=$(oc get openstack -n openstack-operators -o name); then
        ...
        To restore it, use:
        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{\"spec\": {\"operatorOverrides\": [{\"name\": \"manila\", \"replicas\": 1}]}}'\n"

        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{"spec": {"operatorOverrides": [{"name": "manila", "replicas": 0}]}}'
    fi
fi
